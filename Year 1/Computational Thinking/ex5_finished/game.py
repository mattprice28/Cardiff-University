...
def calculate_weight():
    #This function calculates the sum of the weight of the items being carried by the player
    weight = 0
    for index in inventory:
        weight = weight + float(index["mass"])
    return(weight)

def print_inventory_weight():
    weight = calculate_weight()
    print("You are carrying " + str(weight) + "kg of items.")
    print("You cannot carry more than 3kg.")
    print()
...
def print_room_items(room):
    ...
    """
    itemsString = list_of_items(room["items"])
    if itemsString != "":
        print("There is " + itemsString + " here.")
        print()
...
def execute_go(direction):
    ...
    if is_valid_exit(current_room["exits"], direction) == True:
        global current_room
        room_id = ""+ current_room["exits"][direction] +""
...
def execute_take(item_id):
    ...
    """
    listCopy = current_room["items"]
    item_changed = False
    for index in listCopy:
        if index["id"] == item_id:
            finalWeight = float(index["mass"]) + calculate_weight()
            if finalWeight <= 3: 
                ...
                print("Drop some stuff to pick that up.")
                return 
...
def check_if_victory():
    itemCount = 0
    for key in rooms["Reception"]["items"]:
...
def main():
    ...
    while True:
        ...
        if check_if_victory() == False:
            print_room(current_room)
            print_inventory_items(inventory)
            print_inventory_weight()
            print()
            print("Try to drop every item at the reception")
